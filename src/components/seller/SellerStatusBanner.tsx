...
export function SellerStatusBanner() {
  ...
  const bannerConfig = {
    pending: {
      ...
      message: 'Your seller application is being reviewed by our admin team. This usually takes 1-2 business days.',
    },
    approved: {
      icon: CheckCircle,
      bg: 'bg-green-50',
    ...
    rejected: {
      ...
      message: sellerStatus.rejection_reason || 'Your seller application was rejected. Please contact support for more details.',
    },
    suspended: {
      icon: AlertCircle,
      bg: 'bg-orange-50',
      ...
      message: 'Your seller account has been suspended. Please contact support to resolve this issue.',
    },
  };
    ...
    <div className={`${config.bg} border ${config.border} rounded-lg p-4 mb-6`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3 className={`font-semibold ${config.text} mb-1`}>{config.title}</h3>
          ...
          {sellerStatus.status === 'approved' && sellerStatus.approved_at && (
            <p className={`text-xs ${config.text} mt-2`}>
              Approved on {new Date(sellerStatus.approved_at).toLocaleDateString()}
            </p>
          )}
        </div>
      </div>
