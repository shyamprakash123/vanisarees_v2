...
async function createShiprocketOrder(token: string, orderData: ShiprocketOrder) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { orderId, orderNumber, items, shippingAddress, billingAddress, subtotal, weight, dimensions } = await req.json();
    ...
    const billingNameParts = billingAddress ? billingAddress.name.split(" ") : nameParts;
    ...
    const shiprocketOrder: ShiprocketOrder = {
      ...
      billing_address: billingAddress?.address_line1 || shippingAddress.address_line1,
      billing_address_2: billingAddress?.address_line2 || shippingAddress.address_line2 || "",
      billing_city: billingAddress?.city || shippingAddress.city,
      billing_pincode: billingAddress?.postal_code || shippingAddress.postal_code,
      ...
      billing_email: billingAddress?.email || shippingAddress.email || "customer@example.com",
      billing_phone: billingAddress?.phone || shippingAddress.phone,
      shipping_is_billing: !billingAddress,
      order_items: items.map((item: any) => ({
        name: item.title || item.name,
        sku: item.sku || `SKU-${item.id || Math.random().toString(36).substr(2, 9)}`,
        units: item.quantity,
    ...
    if (!shiprocketOrder.shipping_is_billing) {
      ...
      shiprocketOrder.shipping_email = shippingAddress.email || "customer@example.com";
