...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const shipping = 0;
    let wallet_amount = 0;
    if (checkoutData.is_wallet) {
      const { data: walletData, error: walletError } = await supabaseClient
        .from("users")
        .select("wallet_balance")
        .eq("id", user.id)
        .maybeSingle();
      wallet_amount = walletData.wallet_balance;
    }
    ...
    if (checkoutData.payment_method === "prepaid" && total > 0) {
    ...
    let walletId = null;
    if (walletUsed > 0) {
      const newBalance = (wallet_amount || 0) - walletUsed;
      ...
      if (error) {
        ...
        walletId = data.id;
        console.log("Inserted transaction ID:", walletId);
      }
    }
