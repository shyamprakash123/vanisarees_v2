...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      const { seller_id, amount, bank_details }: WithdrawalRequest =
        await req.json();

      if (!seller_id || !amount || !bank_details) {
        return new Response(
          JSON.stringify({
            error: "seller_id, amount, and bank_details are required",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      const { data: seller } = await supabase
        .from("sellers")
        .select("seller_wallet_balance, id")
        .eq("id", seller_id)
        .maybeSingle();
      ...
      if (seller.id !== user.id) {
        ...
        if (!adminUser || !adminUser.email.endsWith("@vanisarees.com")) {
          return new Response(
            JSON.stringify({
              error: "Unauthorized to create withdrawal request",
            }),
            {
              status: 403,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (req.method === "PUT") {
      ...
      const {
        ...
        rejection_reason,
        notes,
      }: WithdrawalUpdate = await req.json();
      ...
      if (action === "approve") {
        ...
        updateData.rejection_reason =
          rejection_reason || "Withdrawal request rejected";
      } else if (action === "complete") {
        if (!transaction_id) {
          return new Response(
            JSON.stringify({
              error: "transaction_id is required for completing withdrawal",
            }),
            {
              status: 400,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
        ...
        const newBalance =
          withdrawalRequest.sellers.seller_wallet_balance -
